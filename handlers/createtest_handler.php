<?php
	...
	*/
	...
	try{
		...
		$idTest=uniqid($_SESSION['data-user']['id'].$randomBytes);
		/*
		...
		*/
		...
		if(isset($_POST['testName'])){
			if(preg_match("/[^A-Za-zА-Яа-яЁё0-9_]/u",$_POST['testName'])){
				header("Location: http://mathtest.rfpgu.ru/createtest.html.php");
			}
		}
		if(isset($_POST['subject'])){
			...
			if(!in_array($_POST['subject'],$result)){
				...
				$idSubject=array_search($_POST['subject'],$result);
			}
		}
		...
		foreach ($_POST as $k1=>$v1){
			if(stristr($k1,'task')){
				...
				if (($task_exist||$file_exist)&&$answer_exist&&$points_exist){
					...
					if (!$db_flags['tests']){
						$db_flags['tests']=1;
						$sql="INSERT INTO tests VALUES(:id,:idUser,:testName,NOW())";
						$pdo->prepare($sql)->execute(['id'=>$idTest,
					...
					if (isset($_FILES[$k1])){
						$sql="UPDATE totaltasktable	SET icontest=1 WHERE id_Test=:idTest AND id_Task=:id_Task";
						$pdo->prepare($sql)->execute(['idTest'=>$idTest,
													  'id_Task'=>$numberTask]);
						$numberOfFile=0;
						$idIcontest=1;
						foreach($_FILES[$k1]['tmp_name'] as $k2=>$v2){
							...
							if ($errorCode !== UPLOAD_ERR_OK || !is_uploaded_file($filePath)) {
								endOfCheck:
									$sql="INSERT INTO icontest VALUES(
										  :idTest,:numberTask,:idIcontest,'Файл отсутсвует по причине ошибки загрузки')";
									$pdo->prepare($sql)->execute(['idTest'=>$idTest,
																  'numberTask'=>$numberTask,
							...
							if(is_uploaded_file($_FILES[$k1]['tmp_name'][$k2]['myPhoto'])){
								mkdir('../user-img/'.$_SESSION['data-user']['id']. DIRECTORY_SEPARATOR .$idTest,0755,true);
								if(move_uploaded_file
									(
										$_FILES[$k1]['tmp_name'][$k2]['myPhoto'],
										__DIR__ . DIRECTORY_SEPARATOR .'..'. DIRECTORY_SEPARATOR .'user-img'. DIRECTORY_SEPARATOR .$_SESSION['data-user']['id']. DIRECTORY_SEPARATOR .$idTest. DIRECTORY_SEPARATOR .$name.$format
									)){
									$sql="INSERT INTO icontest VALUES(
										  :idTest,:numberTask,:idIcontest,:myPhoto)";
		...
		if ($numberTask>1){
			$sql="INSERT INTO tasktest VALUES(:id_Test,:countTask,:mark_1,:mark_2,:mark_3,:mark_4,:mark_5)";
			$pdo->prepare($sql)->execute(['id_Test'=>$idTest,
			...
			$sql="INSERT INTO `testSubject` VALUES(:idSubject,:idTest)";
			$pdo->prepare($sql)->execute(['idSubject'=>($idSubject+1),
										  'idTest'=>$idTest]);
			
			if(isset($_POST['section'])&&$_POST['section']!==''){
				$sql="SELECT `section` FROM `sections`";
				$result=$pdo->query($sql);
				$result=$result->fetchAll(PDO::FETCH_ASSOC);
				for($i=0;$i<count($result);$i++){
					$result[$i]=$result[$i]['section'];
				}
				if(in_array($_POST['section'],$result)){
					$idSection=array_search($_POST['section'],$result);
					$sql="INSERT INTO `testSection` VALUES(:idSection,:idTest)";
					$pdo->prepare($sql)->execute(['idSection'=>($idSection+1),
												  'idTest'=>$idTest]);
				}
			}
		}
		...
		if ($idTest!==null){
			if (file_exists("../user-img/".$_SESSION['data-user']['id']."/".$idTest)){
				rmdir("../user-img/".$_SESSION['data-user']['id']."/".$idTest);
			}
		}
		$pdo->rollBack();
